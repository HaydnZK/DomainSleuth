...
def get_remediation(finding):
    if finding["type"] == "GenericAll":
        ...
        return {
            ...
            "related_nodes": finding.get("path") if isinstance(finding.get("path"), list) else [finding.get("path")]
        }
        ...
        return {
            ...
            "exploitability": get_exploitability(finding["type"]),
            "description": f"Group {finding.get('group')} contains excessive members: {finding.get('member_count')}",
...
def generate_pdf(results, attack_paths, findings):
    ...
    for r in results:
        if r["severity"] == "Critical":
            elements.append(Paragraph(f"• {r['title']}", styles["BodyText"]))
    ...
    if attack_paths:  # only show if list is NOT empty
        ...
        for i, ap in enumerate(attack_paths):
            elements.append(Paragraph(ap, styles["BodyText"]))

            f = [x for x in findings if x["type"] == "AttackPath"][i]
            src, dst = get_first_step(f["path"])

            if src and dst:
                elements.append(Paragraph(
                    f"<b>Remediation Insight:</b> Restrict access between {src} and {dst} to break this attack chain.", styles["BodyText"]))
    ...
    for i, r in enumerate(results):
        ...
        if current_score == new_score:
            elements.append(Paragraph(f"<b>Fix Impact:</b> No immediate score reduction, but reduces attack surface and contributes to breaking escalation paths.", styles["BodyText"]))
        else:
            elements.append(Paragraph(f"<b>Fix Impact:</b> Reduces risk score from {current_score} ({current_level}) → {new_score} ({new_level})", styles["BodyText"]))
...
def manual_input():
    ...
    while True:
        ...
        if f_type == "GenericAll":
            ...
            member_count = input("Enter members (comma separated): ").split(",")
    ...
    for r in results:
        ...
        for item in related:
            if isinstance(item, list):
                flat_related.extend([str(x) for x in item])
            else:
                flat_related.append(str(item))

        if flat_related:
            print("Related Nodes:", ", ".join(flat_related))
        else:
